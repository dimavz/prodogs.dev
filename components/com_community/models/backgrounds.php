...
class CommunityModelBackgrounds extends JCCModel {
    ...
    public function __construct() {
        ...
        {
            // legacy - predefined (non-custom) moods use untraslated background strings as identifiers
            if(!$background->custom) {
                $background->id = $background->title;
                $background->title = JText::_('COM_COMMUNITY_MOOD_SHORT_' . strtoupper($background->title));
            }
            ...
            if($background->custom) {
                ...
                if( file_exists(COMMUNITY_STATUS_BACKGROUND_PATH.$imagefilename)) {
                    $background->image = JUri::root().str_replace(JPATH_ROOT,'',COMMUNITY_STATUS_BACKGROUND_PATH).$imagefilename;
                } else {
                    $background->image ='';
                }

                if( file_exists(COMMUNITY_STATUS_BACKGROUND_PATH.$thumbfilename)) {
                    $background->thumb = JUri::root().str_replace(JPATH_ROOT,'',COMMUNITY_STATUS_BACKGROUND_PATH).$thumbfilename;
                } else {
                    $background->thumb ='';
                }
            }
