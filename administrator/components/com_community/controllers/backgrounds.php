...
class CommunityControllerBackgrounds extends CommunityController
{
    ...
    public function apply()
    {
        ...
        $mainframe->redirect('index.php?option=com_community&view=backgrounds&layout=edit&moodid='.$background->id, $background->message, 'message');
    }
    ...
    public function store()
    {
        ...
        if (!empty($backgroundImage['tmp_name']) && isset($backgroundImage['name']) && !empty($backgroundImage['name'])) {
            ...
            CImageHelper::resizeProportional($finalPath, $finalPath, "image/$ext", 800);

            //add another copy of the background image, during reinstallation or upgrade, so that the default background wont be replaced.
            $copyImgPath = $imagePath.'/background_'.$background->id."_new.".$ext;
            copy($finalPath, $copyImgPath);
            CImageHelper::resizeProportional($copyImgPath, $copyImgPath, "image/$ext", 800);
